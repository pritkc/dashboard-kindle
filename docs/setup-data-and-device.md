...
The simulator enrolls itself when no `DASHBOARD_KINDLE_DEVICE_TOKEN` is set, downloads the assigned PNG, validates the PNG signature, validates `X-Image-SHA256`, stores the image under `data/artifacts/simulator`, and writes the latest ETag.
